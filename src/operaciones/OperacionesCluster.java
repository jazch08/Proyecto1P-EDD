...
public class OperacionesCluster {
    ...
    private static LinkedList<Pixel> crearCluster(Pixel pixel, HashMap<Punto, Pixel> matrizColores){
        ...
        do{
            ...
            if((punto.getX()+1)<=Lectura.filas && matrizColores.get(new Punto(punto.getX()+1,punto.getY())).getCodColor().equals(piTemp.getCodColor())){    
            ...
            else if((punto.getY()-1)>=0 && matrizColores.get(new Punto(punto.getX(),punto.getY()-1)).getCodColor().equals(piTemp.getCodColor())){    
                Pixel p = matrizColores.get(new Punto(punto.getX(),punto.getY()-1));
                pila.push(p);
                camino.add(p);
                pixelesUsados.add(p);
